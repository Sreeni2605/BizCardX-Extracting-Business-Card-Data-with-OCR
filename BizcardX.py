...
if select == "Home":
  ...
  st.markdown('<h2 style="text-align: center; color: #FF7F50;">BizCardX : Extracting Business Card Data with OCR</h2>', unsafe_allow_html=True)
  ...
  with col2:
    ...
    st.markdown("Bizcard is a Python application designed to extract information from business cards. The main purpose of Bizcard is to automate the process of extracting key details from business card images, such as the name, designation, company, contact information, and other relevant data. By leveraging the power of OCR (Optical Character Recognition) provided by EasyOCR, Bizcard is able to extract text from the images.")

if select == "Upload":
  

  st.markdown('<h2 style="text-align: center; color: #FF7F50;">Image Processing & Inserting in DataBase</h2>', unsafe_allow_html=True)
  st.markdown("<hr style='border: 2.5px solid #7FD8BE;'>", unsafe_allow_html=True)

  img = st.file_uploader("**The data will be extracted once you upload the image.**", type=["png","jpg","jpeg"])
  ...
  if img is not None:
    ...
    with col2:
      ...
      if button_clicked:
          ...
          mydb = psycopg2.connect("bizcardx.db")
          cursor = mydb.cursor()
...
if select == "Preview":
  ...
  st.markdown('<h2 style="text-align: center; color: #FF7F50;">Database-Storage of Business Card Information</h2>', unsafe_allow_html=True)
  st.markdown("<hr style='border: 2.5px solid #7FD8BE;'>", unsafe_allow_html=True)
...
if select == "Modify":
  ...
  if button3:
      conn = psycopg2.connect('bizcardx.db')
      cursor = conn.cursor()
